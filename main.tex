...
A) Effect of incubation temperature, time and addition of a phosphatase (rSAP) using the l3N adapter to ligate purified human tRNA. B) Effect of additives using 4C, 24 h incubation and the l2N adapter to ligate purified human tRNA. An irrelevant well has been crossed out. C) Effect of ligase type using the l1N adapter to ligate the tRNALys oligo. Some wells have been added an additional amount of non-adenylated adapter equal to the amount of pre-adenylated adapter added (indicated by l1N in well label).
...
A) Percentage of spiked-in E. coli tRNALys oligo that was sequenced as ending on CCA and thus wrongly assigned as aminoacylated. B) Aminoacylation level of tRNA transcripts after undergoing deacylation by incubation at 45C for 4 h in 1 M lysine (pH=8). On the right, the barcode average is plotted for each transcript. In these plots the mitochondrial tRNAfMet was excluded because its deacylation is known to be incomplete due to its high acylation half-life. C) Aminoacylation level of tRNA transcripts from four samples receiving sham oxidation during the Whitfeld reaction.
...
Errorbars are bootstrapped 95\% confidence interval over the 7 individual samples barcoded, pooled and used for RT-PCR template with both TGIRT-III and Maxima.
