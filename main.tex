...
\subsubsection{Splint assisted ligation}
...
\figsupp[Measurement bias in charge tRNAseq using blunt-end ligation.]{
Measurement bias in charge tRNAseq using blunt-end ligation.
...
Using deacylated and gel purified human tRNA as substrate and pre-adenylated l3N as adapter, otherwise following the method as \cite{Behrens2021-gb}.
...
Charge tRNAseq control samples and spike-ins conform to 
...
\figsupp[Suppl. fig 7.]{
\textbf{(A)} The Maxima RT polymerase (Thermo) produces similar levels of full size cDNA as TGIRT-III under the standard tRNAseq RT-PCR conditions (42°C, 16 h, suggested by \cite{Behrens2021-gb}).
...
\textbf{(B)} tRNAseq DNA library reannealing is inhibited by high salt concentrations.
A gel purified charge tRNAseq DNA library was resuspended in TBE buffer and incubated for 30 minutes at different temperatures with or without 1 M NaCl.
...
Ligations were then pooled and submitted to the remainder of the charge tRNAseq protocol.
\textbf{(A)} The percentage point deviation from the median codon charge, grouped by barcode identity (left) or shown summarized as a density plot (right).
\textbf{(B)} The percentage deviation from the median codon RPM, grouped by barcode identity (left) or shown summarized as a density plot (right).
...
For barcode replicate test (Fig4).
tRNA from 8h incubation condition was used because it contains a spectrum of charge levels spanning almost fully charged isoleucine tRNAs and almost fully deacylated asparagine tRNA.
...
Grid search.
Unique transcript is not an optimal criteria because it can easily lead to masking parameters adjusting to mask a single transcript out of a family of highly similar transcripts.
This can lead to the false confidence in assignment of annotation to truncated reads that cannot truly distinguish between this family of highly similar transcripts.
